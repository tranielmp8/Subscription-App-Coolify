...
<div class="mx-auto max-w-2xl">
	<div class="mb-6">
		<a href="/subscriptions/{sub.id}" class="text-sm text-indigo-600 hover:underline">← Back to {sub.name}</a>
		<h1 class="mt-2 text-2xl font-bold text-gray-900">Edit Subscription</h1>
	</div>
	...
	<form method="POST" use:enhance class="space-y-6">
					...
					<div>
						...
						>
							{#each CATEGORIES as cat}
								<option value={cat} selected={sub.category === cat}>
		...
		<section class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
			...
			<p class="mb-4 text-xs text-gray-400">Passwords are encrypted at rest.</p>
			<div class="space-y-4">
				<div>
					...
					<div class="grid gap-4 sm:grid-cols-2">
						<div>
							...
							<input
								...
								type="time"
								value={notifications?.notifyTime ?? '08:00'}
								step="900"
								class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 focus:outline-none"
							/>
						</div>
